...
// 페이지네이션 코드는 AI로 제작하였습니다. 코드 시작
let currentPage = 1;
let currentList = contentsListData;
const ITEMS_PER_PAGE = 10;
const $paginationCt = document.querySelector('.tc-bottom-number');
...
function renderPagination(totalItems) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        $page.textContent = i;
        if (i === currentPage) $page.classList.add('tc-bottom-active-number');
...
function userTotalManagement(arrayList) {
    ...
    for (let i = pageMaxArray - 1; i >= 0; i--) {
        ...
        <p data-contentId="${pageItems[i].contentId}"> ${pageItems[i].contentTitle} </p>
...
function deleteContents() {
    ...
    const totalPages = Math.ceil(contentsListData.length / ITEMS_PER_PAGE);
    if (currentPage > totalPages) currentPage = totalPages || 1;
    currentList = contentsListData;
    userTotalManagement(contentsListData);
}
